...
def original_error_function(j):#傳入x0 x1 x2 y
	#return np.power(sigmoid(j[0]), j[1]) + np.power((1-sigmoid(j[0])), j[1])
	#a = sigmoid(j[0])
	#temp = -(j[1] * np.log(a+1e-15) + (1 - j[1]) * np.log(1-a+1e-15))
	#return temp
...
while epoch < 100000:
	...
	for i in Case4_Training: # Case4_Training每個都做一遍
		...
		#print(original_error_function(i))
		#if not cross_entropy_error(i): # Case1_Data每個都做一遍
		W = W + eta * np.dot((i[1]-sigmoid(i[0])), i[0]) # 調整W
			#check = False # W還需要調整
	epoch += 1
	print("W: \n", W)
	print("epoch: ", epoch)
	print("xxxxxxxxxxxxxxxxxxxxxx") # 'x'*10
	print(sum_sqerror / len(Case4_Training))
	if (sum_sqerror / len(Case4_Training)) < tau:
		break
